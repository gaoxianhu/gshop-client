        ...
        <div class="orders">
            ...
            <table class="order-item" v-for="order in orders" :key="order.id">
                <thead>
                    <tr>
                        <th colspan="5">
                            <span class="ordertitle">{{order.createTime}}　订单编号：{{order.outTardeNo}} <span
                ...
                <tbody>
                    <tr v-for="(item, index) in order.orderDetailList" :key="item.id">
                        ...
                        <!-- 
                            子模版：组件<template>内部的</template>
                                它再模板解决过程中存在，不会显示到页面上
                            利用它可以做什么？ 可以用来统一控制多个标签
                         -->
